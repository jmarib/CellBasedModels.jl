...
@testset verbose=verbose "abstractTypes Integrators" begin
    ...
    @testset "Rule" begin
        ...
        r3 = Rule(:c, :dt, ex, ex, ex)
        @test r3 isa Rule
        @test r3.rules[2] === ex

        rkw = Rule(:c, :dt, f=(ex, ex))
        @test rkw isa Rule
        @test rkw.rules == (ex, ex)

        @test_throws ErrorException Rule(:c, :dt, 1)  # non-Expr positional should error (your original API)
    end
    ...
    @testset "ODE" begin
        ...
        okw = ODE(:c, :dt, f = (ex, ex))
        @test okw isa ODE
        @test okw.f[2] === ex
    end

    @testset "DynamicalODE" begin
        d = DynamicalODE(:c, :dt, f1 = ex, f2 = (ex, ex))
    ...
    @testset "SplitODE" begin
        s = SplitODE(:c, :dt, f1 = (ex, ex), f2 = ex)
        @test s isa SplitODE
        @test s.f1[1] === ex
        @test s.f2 == (ex,)
    end
    ...
    @testset "ADIODE (1/2/3 parts)" begin
        # 1-part + g
        a1 = ADIODE(:c, :dt, f1 = ex, g = ex)
        @test typeof(a1) === ADIODE{1, true}
        @test a1.f1 == (ex,)
        @test a1.g  == (ex,)

        # no g provided -> empty g tuple
        a1b = ADIODE(:c, :dt, f1 = (ex, ex))
        @test typeof(a1b) === ADIODE{1, false}
        @test a1b.g === nothing

        # 2-parts + g
        a2 = ADIODE(:c, :dt, f1 = ex, f2 = (ex, ex), g = (ex, ex))
        @test typeof(a2) === ADIODE{2, true}
        @test a2.f2 == (ex, ex)
        @test a2.g  == (ex, ex)

        # 3-parts, empty g
        a3 = ADIODE(:c, :dt, f1 = ex, f2 = ex, f3 = (ex, ex))
